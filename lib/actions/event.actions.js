...
export const extractPosterInfo = async (imageUrl) => {
  ...
  const SIMULATED_ERROR = false;
  const DUMMY_RESPONSE = false;

  if (SIMULATED_ERROR) {
    const errorResponse = new Promise((_, reject) => {
      setTimeout(() => {
        reject(new Error("Simulated error in poster info extraction"));
      }, 2000);
    });
...
export async function createEvent({ userId, event, path }) {
  try {
    ...
    const newEvent = await Event.create({
      ...
      organizer: userId,
      organizerClerkId: organizer.clerkId,
    });
    revalidatePath(path);
...
// DELETE
export async function deleteEvent({ eventId, userId, organizer, path }) {
  try {
    await connectToDatabase();
